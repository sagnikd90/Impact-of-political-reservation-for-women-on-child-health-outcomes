...
stargazer(reg_1,reg_2,reg_3,reg_4,reg_5,reg_6,reg_7,reg_8,
          ...
          order = c("^treat$",
                    ...
                    "^mothers_age_at_birth$",
                    "^wait_time$",
                    "^factor(birth_order)2$",
                    "^factor(birth_order)3$"),
          covariate.labels = c("Post-treat",
                               ...
                               "(Birth order=2 vs 1)",
                               "(Birth order=3 vs 1)"),
          add.lines = list(c("Individual controls","x","x","x","x","x","x","x","x"),
                           c("State FE","x","x","x","x","x","x","x","x"),
                           c("Cohort FE","x","x","x","x","x","x","x","x")),
          no.space = TRUE,
          omit.stat = c("ser","f","adj.rsq"),
          out = "Baseline_reg_1.tex")
...
reg_5 <- glm(data = df_second_third,
             formula = neonat~factor(birth_order)*treat+
               state_name+
               factor(cohort)+
               sex+
...
stargazer(reg_1,reg_2,reg_3,reg_4,reg_5,reg_6,
          ...
          add.lines = list(c("Time-trend","","x","","x","","x"),
          ...
          omit.stat = c("ll","aic"),
          no.space = TRUE,
          out = "Neonatal_reg_all.tex")

##################################### Neonatal survival probability when first born is a boy ###################################
...
reg_2 <- glm(data = df_first_second_boys,
             formula = neonat~factor(birth_order)*treat+
               state_name+
               factor(cohort)+
               sex+
...
reg_4 <- glm(data = df_first_third_boys,
             formula = neonat~factor(birth_order)*treat+
               state_name+
               factor(cohort)+
               sex+
...
stargazer(reg_1,reg_2,reg_3,reg_4,reg_5,reg_6,
          ...
          omit.stat = c("ll","aic"),
          no.space = TRUE,
          out = "Neonatal_reg_1st_boy.tex")


##################################### Neonatal survival probability when first two born are boys ###################################
...
reg_2 <- glm(data = df_first_third_2_boys,
             formula = neonat~factor(birth_order)*treat+
               state_name+
               factor(cohort)+
               sex+
...
reg_3 <- glm(data = df_second_third_2_boys,
             formula = neonat~factor(birth_order)*treat+
               state_name+
               factor(cohort)+
               sex+
...
stargazer(reg_1,reg_2,reg_3,reg_4,
          ...
          p=c(pval_1,pval_2,pval_3,pval_4),
          omit = c("state_name",
                   "cohort",
                   "read_write_woman",
                   "caste",
          ...
          covariate.labels = c("Sex of the child",
                               ...
                               "Post-treat",
                               "(Birth order=3 vs 1)*post-treat"),
          add.lines = list(c("Time trend","","x","","x"),
                           c("Individual controls","x","x","x","x"),
                           c("State FE","x","x","x","x"),
                           c("Cohort FE","x","x","x","x"),
                           c("R-sq",rsq_1,rsq_2,rsq_3,rsq_4)),
          no.space = TRUE,
          omit.stat = c("aic","ll"),
          out = "Neonatal_reg_2_boys.tex")

########################### Neonatal survival probability when first two born are girls ###################################
...
reg_2 <- glm(data = df_first_third_2_girls,
             formula = neonat~factor(birth_order)*treat+
               state_name+
               factor(cohort)+
               sex+
...
reg_3 <- glm(data = df_second_third_2_girls,
             formula = neonat~factor(birth_order)*treat+
               state_name+
               factor(cohort)+
               sex+
...
stargazer(reg_1,reg_2,reg_3,reg_4,reg_5,reg_6,
          ...
          omit.stat = c("ll","aic"),
          no.space = TRUE,
          out = "Infantal_reg_all.tex")

##################################### Infantal survival probability when first born is a boy ###################################
...
stargazer(reg_1,reg_2,reg_3,reg_4,reg_5,reg_6,
          ...
          omit.stat = c("ll","aic"),
          no.space = TRUE,
          out = "Infantal_reg_1st_boy.tex")


##################################### Infantal survival probability when first two born are boys ###################################
...
stargazer(reg_1,reg_2,reg_3,reg_4,
          ...
          no.space = TRUE,
          omit.stat = c("aic","ll"),
          out = "Infantal_reg_2_boys.tex")

########################### Infantal survival probability when first two born are girls ###################################
...
stargazer(reg_1,reg_2,reg_3,reg_4,
          ...
          no.space = TRUE,
          omit.stat = c("aic","ll"),
          out = "Infantal_reg_2_girls.tex")

########################################## Under 5 years survival probability ###################################
...
stargazer(reg_1,reg_2,reg_3,reg_4,reg_5,reg_6,
          ...
          omit.stat = c("ll","aic"),
          no.space = TRUE,
          out = "Under5_reg_all.tex")

##################################### Under5al survival probability when first born is a boy ###################################
...
stargazer(reg_1,reg_2,reg_3,reg_4,reg_5,reg_6,
          ...
          omit.stat = c("ll","aic"),
          no.space = TRUE,
          out = "Under5_reg_1st_boy.tex")


##################################### Under5al survival probability when first two born are boys ###################################
...
stargazer(reg_1,reg_2,reg_3,reg_4,
          ...
          no.space = TRUE,
          omit.stat = c("aic","ll"),
          out = "Under5_reg_2_boys.tex")

########################### Under5al survival probability when first two born are girls ###################################
...
stargazer(reg_1,reg_2,reg_3,reg_4,
          ...
          no.space = TRUE,
          omit.stat = c("aic","ll"),
          out = "Under5_reg_2_girls.tex")


########################################## Under 10 years survival probability ###################################
...
stargazer(reg_1,reg_2,reg_3,reg_4,reg_5,reg_6,
          ...
          omit.stat = c("ll","aic"),
          no.space = TRUE,
          out = "Under10_reg_all.tex")

##################################### Under10al survival probability when first born is a boy ###################################
...
stargazer(reg_1,reg_2,reg_3,reg_4,reg_5,reg_6,
          ...
          omit.stat = c("ll","aic"),
          no.space = TRUE,
          out = "Under10_reg_1st_boy.tex")


##################################### Under10al survival probability when first two born are boys ###################################
...
stargazer(reg_1,reg_2,reg_3,reg_4,
          ...
          no.space = TRUE,
          omit.stat = c("aic","ll"),
          out = "Under10_reg_2_boys.tex")

########################### Under10al survival probability when first two born are girls ###################################
